...
class CellGraph(object):
    ...
    def __init__(self, formula: QFFormula,
        ...
        for pred in self.preds:
            if pred.name.startswith('@A'):
                target_A_pred = pred
                break
        target_X_preds: list[Pred] = [pred for pred in self.preds if pred.name.startswith('@X')]
        target_X_preds.sort(key=lambda x: int(x.name[2:]))
        if target_A_pred is not None:
            self.cells.sort(key=lambda x: [x.is_positive(target_A_pred),] 
                                            + [x.is_positive(p) for p in target_X_preds], 
                            reverse=True)
...
class OptimizedCellGraphWithPC(CellGraph):
    def __init__(self, formula: QFFormula,
        ...
        logger.info(f"Final i1 independent cliques: {self.i1_ind}")
        logger.info(f"Final non-independent cliques: {self.nonind}")
        self.term_cache = dict()
